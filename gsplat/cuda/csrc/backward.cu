...
__global__ void rasterize_backward_2d_bg_kernel(
    const dim3 tile_bounds,
    const dim3 img_size,
    const int32_t* __restrict__ gaussian_ids_sorted,
    const int2* __restrict__ tile_bins,
    ...
    const float3 bg = background[pix_id];
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        for (int t = max(0,batch_end - warp_bin_final); t < batch_size; ++t) {
            ...
            float2 v_xy_local = {0.f, 0.f};
            float v_opacity_local = 0.f;
            float3 v_background_local = {0.f, 0.f, 0.f};
            //initialize everything to 0, only set if the lane is valid
            if(valid){
                // compute the current T for this gaussian
                float ra = 1.f / (1.f - alpha);
                ...
                v_xy_local = {v_sigma * (conic.x * delta.x + conic.y * delta.y), 
                                    v_sigma * (conic.y * delta.x + conic.z * delta.y)};
                v_opacity_local = vis * v_alpha;
                v_background_local = {T_final * ra * v_out.x, T_final * ra * v_out.y, T_final * ra * v_out.z};
            }
            warpSum3(v_rgb_local, warp);
            warpSum3(v_conic_local, warp);
            warpSum2(v_xy_local, warp);
            warpSum(v_opacity_local, warp);
            warpSum3(v_background_local, warp);
            if (warp.thread_rank() == 0) {
                int32_t g = id_batch[t];
                float* v_rgb_ptr = (float*)(v_rgb);
                atomicAdd(v_rgb_ptr + 3*g + 0, v_rgb_local.x);
                ...
                float* v_background_ptr = (float*)(v_background);
                atomicAdd(v_background_ptr + 3 * pix_id + 0, v_background_local.x);
                atomicAdd(v_background_ptr + 3 * pix_id + 1, v_background_local.y);
                atomicAdd(v_background_ptr + 3 * pix_id + 2, v_background_local.z);
            }
        }
    }
}
