...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
rasterize_forward_2d_bg_tensor(
    const std::tuple<int, int, int> tile_bounds,
    const std::tuple<int, int, int> block,
    const std::tuple<int, int, int> img_size,
    const torch::Tensor &gaussian_ids_sorted,
    ...
    rasterize_forward_2d_bg<<<tile_bounds_dim3, block_dim3>>>(
        tile_bounds_dim3,
        img_size_dim3,
        gaussian_ids_sorted.contiguous().data_ptr<int32_t>(),
        (int2 *)tile_bins.contiguous().data_ptr<int>(),
...
std::
    tuple<
        ...
        >
    rasterize_backward_2d_bg_tensor(
        const unsigned img_height,
        const unsigned img_width,
        const unsigned block_width,
        const torch::Tensor &gaussians_ids_sorted,
    ...
    if (background.ndimension() != 2 || background.size(1) != 3) {
        AT_ERROR("background must have 2 dimensions");
    }
    ...
    torch::Tensor v_colors =
        torch::zeros({num_points, channels}, xys.options());
    torch::Tensor v_opacity = torch::zeros({num_points, 1}, xys.options());
    torch::Tensor v_background = torch::zeros({img_height*img_width, channels}, xys.options());

    rasterize_backward_2d_bg_kernel<<<tile_bounds, block>>>(
        tile_bounds,
        img_size,
        gaussians_ids_sorted.contiguous().data_ptr<int>(),
        (int2 *)tile_bins.contiguous().data_ptr<int>(),
